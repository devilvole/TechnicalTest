	...
	public class CustomerService : ICustomerService
	{
		...
		public async Task<IResult> GetTransfers(string customerName)
		{
			var customer = await db.Customers
				.Include(r => r.AccountTransfers)
				.ThenInclude(x => x.SourceAccount)
				.Include(r => r.AccountTransfers)
				.ThenInclude(x => x.DestinationAccount)
			...
			var transfers = customer.AccountTransfers
				.Select(x => new
				{
					Source = x.SourceAccount?.AccountNumber,
					Destination = x.DestinationAccount?.AccountNumber,
					Amount = x.Amount,
